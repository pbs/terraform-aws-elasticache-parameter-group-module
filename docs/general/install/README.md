...
module "elasticache-parameter-group" {
    source = "github.com/pbs/terraform-elasticache-parameter-group-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/elasticache-parameter-group'
gh -R 'pbs/terraform-elasticache-parameter-group-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/elasticache-parameter-group'
rm -f release.tar.gz
```
...
```bash
git remote add -f elasticache-parameter-group git@github.com:pbs/terraform-elasticache-parameter-group-module.git
git subtree add --prefix terraform/modules/elasticache-parameter-group elasticache-parameter-group main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/elasticache-parameter-group elasticache-parameter-group $REF --squash
```
